...
The `operations/` and `repos/` folders ground Pillar 3's golden paths and guardrails. The service mapping defines dependencies, the repo index inventories every component, and the runbook codifies operational readiness. Cross-file consistency checks, such as whether every repository in `repos/index.yaml` appear in `operations/service-mapping.yaml`, to replace the manual coordination that typically falls through the cracks between architecture, development, and operations teams.
